...
CREATE TABLE t (
	...
	idx := ti.Indices[0]
	info := common.WrapTableInfo("test", ti)
	rewritten := rewriteColsOffset(idx, info)
	require.Equal(t, idx, rewritten)

	// check GetWhereHandle when target is same as source
	handle := GetWhereHandle(info, info)
...
CREATE TABLE t (
	...
	rewritten = rewriteColsOffset(targetIdx, info)
	...
	targetInfo := common.WrapTableInfo("test", targetTI)
	handle = GetWhereHandle(info, targetInfo)
	require.Len(t, handle.UniqueNotNullIdx.Columns, 2)
	require.Equal(t, handle.UniqueNotNullIdx.Columns[0].Offset, 1)
	require.Equal(t, handle.UniqueNotNullIdx.Columns[1].Offset, 2)
