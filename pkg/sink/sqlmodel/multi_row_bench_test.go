...
func prepareDataMultiColumnsPK(t *testing.T, batch int) []*RowChange {
	...
	for i := 0; i < batch; i++ {
		change := NewRowChange(source, target,
			[]interface{}{i + 1, i + 2, i + 3, i + 4, "c5", "c6", "c7", time.Time{}, time.Time{}},
			[]interface{}{i + 10, i + 20, i + 30, i + 40, "c5", "c6", "c7", time.Time{}, time.Time{}},
			sourceTI, targetTI, nil)
		changes = append(changes, change)
	}
	return changes
...
// bench cmd: go test -run='^$' -benchmem -bench '^(BenchmarkGenUpdate)$' github.com/pingcap/ticdc/pkg/sink/sqlmodel
func BenchmarkGenUpdate(b *testing.B) {
	...
	benchCases := []genCase{
		{
			name:    "OneColumnPK-GenUpdateSQL-v1",
			fn:      GenUpdateSQL,
			prepare: prepareDataOneColoumnPK,
		},
		{
			name:    "OneColumnPK-GenUpdateSQL-v2",
			fn:      GenUpdateSQL,
			prepare: prepareDataOneColoumnPK,
		},
		{
			name:    "MultiColumnsPK-GenUpdateSQL-v1",
			fn:      GenUpdateSQL,
			prepare: prepareDataMultiColumnsPK,
		},
		{
			name:    "MultiColumnsPK-GenUpdateSQL-v2",
			fn:      GenUpdateSQL,
			prepare: prepareDataMultiColumnsPK,
		},
	}
	for _, bc := range benchCases {
		for _, batch := range batchSizes {
			...
			b.Run(name, func(b *testing.B) {
				changes := bc.prepare(t, batch)
				whereClause := "v2"
				if strings.HasSuffix(bc.name, "-v1") {
					whereClause = "v1"
				}
				for i := 0; i < b.N; i++ {
					bc.fn(whereClause, changes...)
				}
			})
		}
	}
